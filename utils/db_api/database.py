...
class UserDatabase(Database):
    ...
    def add_user_if_not_exists(self, telegram_id: int, username: str):
        """
        Foydalanuvchi mavjud bo'lmasa qo'shadi, mavjud bo'lsa hech narsa qilmaydi.
        True - yangi foydalanuvchi qo'shildi, False - allaqachon bor edi.
        """
        user = self.select_user(telegram_id)
        if not user:
            self.add_user(telegram_id, username)
            return True
        return False
